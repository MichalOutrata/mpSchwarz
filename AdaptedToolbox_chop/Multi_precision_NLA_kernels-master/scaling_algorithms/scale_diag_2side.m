...
%%% adapted code by Outrata, 2024 -> sparse version
d1 = 1./max(abs(A),[],2); D1 = spdiags(d1,0,length(d1),length(d1));
B = D1*A;  
d2 = 1./max(abs(B),[],1); D2 = spdiags(d2',0,length(d2),length(d2));
B = B*D2;
