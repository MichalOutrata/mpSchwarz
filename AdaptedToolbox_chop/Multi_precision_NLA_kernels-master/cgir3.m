function [x,iter,pcg_its] = cgir3(A,b,precf,precw,precr,iter_max,...
                                    gtol,scale,figs)
...
if (scale.flag == 1 && ufs == 'h')
    ...
    LL = (1/sqrt(mu))*diag(1./diag(R))*double(LL);
    U = (1/sqrt(mu))*double(U)*diag(1./diag(C));
...
%         end
    ...
    elseif (precw == 2 && precr == 2)
        [d, err, its, ~] = pcg_dd(A,zeros(n,1),double(rd1),LL,gtol);
    end
    pcg_its = pcg_its+its;
