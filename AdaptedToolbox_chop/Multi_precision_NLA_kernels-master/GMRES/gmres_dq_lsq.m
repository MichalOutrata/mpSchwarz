function [x, error, its, flag] = gmres_dq_lsq( A, x, b, L, U, restrt, max_it, tol)
%GMRES_DQ_LSQ   Left-preconditioned GMRES in double/quad precision
%   Solves (A^{T}A)x=b by solving the preconditioned linear system (LU)^{-1}(A^{T}A)x=(LU)^{-1}b
...
%           L        REAL L factor of lu(A^{T}A)
%           U        REAL U factor of lu(A^{T}A)
...
for iter = 1:max_it,                              % begin iteration
    ...
    for i = 1:m,                     % construct orthonormal basis via GS
        ...
        vcur = mp(double(U),34)\(mp(double(L),34)\(mp(double(A'),34)*(mp(double(A),34)*mp(double(vcur),34))));
