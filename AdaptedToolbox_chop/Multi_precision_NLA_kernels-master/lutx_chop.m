...
%   It requires the 'chop_dense' function to simulate lower precision arithmetic.
%   By default half precision is used

if nargin<2, format = 'h'; end
fp.format = format; chop_dense([],fp);
...
for k = 1:n-1
   ...
   if (A(ind,k) ~= 0)
      ...
      % Compute multipliers
      i = k+1:m;
      A(i,k) = chop_dense(A(i,k)/A(k,k));

      % Update the remainder of the matrix
      j = k+1:n;
      A(i,j) = chop_dense(A(i,j) - chop_dense(A(i,k)*A(k,j))); 
   end
end
