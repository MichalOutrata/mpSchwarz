function [opdata,x,iter,gmresits] = gmreslsir3(A,b,precf,precw,precr,iter_max,gtol,ptype)
...
nrhs = double(norm(Aug_rhs,inf));
    ...
    if precw == 2
        [d, err, its, flag] = lsqgmres_dq(A,zeros(m+n,1),(Aug_rhs/nrhs),...
            Q1,R,alpha,m+n,1,gtol,ptype);
    else
        [d, err, its, flag] = lsqgmres_sd(A,zeros(m+n,1),(Aug_rhs/nrhs),...
            Q1,R,alpha,m+n,1,gtol,ptype);
    end
