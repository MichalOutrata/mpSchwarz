...
fp.format = 'bfloat16'; [c,options] = chop_dense(pi,fp);
assert_eq(fp.format,options.format)
assert_eq(options.subnormal,0)

fp.format = []; [c,options] = chop_dense(pi,fp);
assert_eq(options.format,'h')  % Check default;

fp.subnormal = 0; [c,options] = chop_dense(pi,fp);
assert_eq(options.subnormal,0)

fp.subnormal = []; [c,options] = chop_dense(pi,fp);
assert_eq(options.subnormal,1)  % Check default;

fp.round = []; [c,options] = chop_dense(pi,fp);
assert_eq(options.round,1)  % Check default.

fp.flip = []; [c,options] = chop_dense(pi,fp);
assert_eq(options.flip,0)  % Check no default.

fp.explim = []; [c,options] = chop_dense(pi,fp);
assert_eq(options.explim,1)  % Check default.

fp.explim = 0; [c,options] = chop_dense(pi,fp);
assert_eq(options.explim,0)  % Check no default.

clear chop fp options
fp.flip = 1; [~,options] = chop_dense([],fp);
...
c = ones(8,1);
d = chop_dense(c,fp); assert_eq(norm(d-c,1)>0,true);
d = chop_dense(c',fp); assert_eq(norm(d-c',1)>0,true);
fp.p = 0; % No bits flipped.
d = chop_dense(c,fp); assert_eq(d,d);
fp.p = 1; % All bits flipped.
d = chop_dense(c,fp); assert_eq(all(d ~= c),true);

clear chop_dense
[~,fp] = chop_dense;
assert_eq(fp.subnormal,1)
assert_eq(fp.format,'h')
[c,options] = chop_dense(pi);
assert_eq(options.format,'h')
assert_eq(options.subnormal,1)
assert_eq(options.round,1)
assert_eq(options.flip,0)
...
fp.format = 'd'; [c,options] = chop_dense(pi,fp);
...
clear chop_dense
[~,fp] = chop_dense;
fp.format = 'b'; [c,options] = chop_dense(pi,fp);
...
options.format = 'b';
A = magic(4);
C = chop_dense(A,options);
assert_eq(A,C);
B = A + randn(size(A))*1e-12;
C = chop_dense(B,options);
assert_eq(A,C);
A2 = hilb(6); C = chop_dense(A2);

options.format = 'c';
options.params = [8 127];  % bfloat16
C1 = chop_dense(A,options);
assert_eq(A,C1);
C2 = chop_dense(B,options);
assert_eq(A,C2);
assert_eq(C,chop_dense(A2));
...
options2.format = 'h';
A = hilb(6);
[X1,opt] = chop_dense(A,options);
[X2,opt2] = chop_dense(A,options2);
assert_eq(X1,X2)
% assert_eq(chop_dense(A,options),chop_dense(A,options2));
...
options.subnormal = 1;
c = chop_dense(xmin,options); assert_eq(c,xmin)
...
options.explim = 0;
x = xmin/2;  c = chop_dense(x,options); assert_eq(c,x)
x = -xmin/2;  c = chop_dense(x,options); assert_eq(c,x)
x = xmax*2;  c = chop_dense(x,options); assert_eq(c,x)
x = -xmax*2;  c = chop_dense(x,options); assert_eq(c,x)
x = xmins/2; c = chop_dense(x,options); assert_eq(c,x)
x = -xmins/2; c = chop_dense(x,options); assert_eq(c,x)
A = [pi -pi; pi -pi];
C = chop_dense(A,options);
options.explim = 1;
assert_eq(C,chop_dense(A,options));
...
for i = 1:m
x = randn;
options.round = 2; y(i,1) = chop_dense(x,options);
options.round = 3; y(i,2) = chop_dense(x,options);
options.round = 4; y(i,3) = chop_dense(x,options);
...
options.format = 'd'; options.subnormal = 0; chop_dense([],options)
a = chop_dense(pi); assert_eq(a,pi)
options.format = 'd'; options.subnormal = 1; chop_dense([],options)
a = chop_dense(pi); assert_eq(a,pi)

x = pi^2;
clear options
options.format = 'd';
y = chop_dense(x,options);  % Should not change x.
assert_eq(x,y);
options.round = 2;
y = chop_dense(x,options);  % Should not change x.
assert_eq(x,y);
...
cd = chop_dense(ad + 1e-5*double(delta),options);
cs = chop_dense(as + 1e-5*delta,options);
assert_eq(cd,double(cs));

options.format = 'c';
options.params = [11 5];
temp1 = chop_dense(single(pi),options);
options.format = 'h';
temp2 = chop_dense(single(pi),options);
...
assert_eq(chop_dense(x,options), single(2^-3 * (sum(2.^(-[0:10])))))

x = 2^-3 * (sum(2.^(-[0:52])));
assert_eq(chop_dense(x,options), 2^-3 * (sum(2.^(-[0:10]))))

options.format = 's';
x = single(2^-3 * (sum(2.^(-[0:23]))));
assert_eq(chop_dense(x,options), x)

x = 2^-3 * (sum(2.^(-[0:52])));
assert_eq(chop_dense(x,options), 2^-3 * (sum(2.^(-[0:23]))))
...
try
    options.format = 'c';
    options.params = [12 5];
    temp = chop_dense(single(pi),options); % Error - double rounding!
catch
end
assert_eq(temp,0)
try
    options.format = 'c';
    options.params = [26 9];
    temp = chop_dense(pi,options); % Error - double rounding!
catch
end
assert_eq(temp,0)
try
    temp = chop_dense(complex(1,1)); % Error - complex data!
catch
end
assert_eq(temp,0)
