function [c,options] = chop_sparse(x, options)
...
% Modification of code from Nicholas J. Higham and Srikara Pranesh
% https://github.com/higham/chop see the BSD-2-Clause license
% at https://github.com/higham/chop?tab=BSD-2-Clause-1-ov-file#readme
...
if nargin >= 1
    [m,n] = size(x);
    % if (sum(sum(x>0))==0) %%%% commented out by Outrata
    %     c = x; 
    %     return;
    % end
    [row,col,x] = find(x);
...
if sum(isnan(c))||sum(isinf(c))
    disp('Inf or NaN')
    error('chop_sparse: Inf or NaN')
    pause
    warning('chop_sparse: Inf or NaN')
end

c = sparse(row,col,c,m,n);
end
...
% 
% y = abs(values) + (values==0);
% e = floor(log2(y) + 1);
% new_values = pow2(round( pow2(values, t-e) ), e-t);
% 
% c = sparse(row,col,new_values,m,n);
